...
여기에 나열되지 않은 형식이 필요하신가요? sage-wiki는 **외부 파서**를 지원합니다 — stdin을 읽고 일반 텍스트를 stdout에 출력하는 모든 언어의 스크립트입니다. 자세한 내용은 [외부 파서](#외부-파서)를 참조하세요.
...
| `sage-wiki pack install <name\|url>`                                                    | 기여 팩 설치                                 |
| `sage-wiki pack apply <name> [--mode merge\|replace]`                                   | 설치된 팩을 프로젝트에 적용                  |
| `sage-wiki pack remove <name>`                                                          | 프로젝트에서 팩 제거                         |
| `sage-wiki pack list`                                                                   | 적용/캐시/번들 팩 목록                       |
| `sage-wiki pack search <query>`                                                         | 팩 레지스트리 검색                           |
| `sage-wiki pack update [name]`                                                          | 설치된 팩을 최신 버전으로 업데이트           |
| `sage-wiki pack info <name>`                                                            | 팩 상세 정보 표시                            |
| `sage-wiki pack create <name>`                                                          | 새 팩 디렉토리 스캐폴드                      |
| `sage-wiki pack validate [path]`                                                        | 팩 스키마 및 파일 검증                       |
...
팩은 특정 도메인을 위한 온톨로지 유형, 프롬프트, 샘플 소스를 번들하는 설치 가능한 구성 프로필입니다. sage-wiki에는 오프라인으로 작동하는 8개의 번들 팩이 포함되어 있습니다:

| 팩 | 대상 | 주요 온톨로지 |
|---|---|---|
| `academic-research` | 연구자 | cites, contradicts, finding, hypothesis |
| `software-engineering` | 개발팀 | implements, depends_on, adr, runbook |
| `product-management` | PM | addresses, prioritizes, user_story |
| `personal-knowledge` | 노트 관리 | relates_to, inspired_by, fleeting_note |
...
sage-wiki에는 12개 이상의 형식에 대한 내장 파서가 있습니다. 그 외의 형식은 모든 언어로 스크립트를 작성하여 외부 파서를 추가할 수 있습니다. stdin/stdout 프로토콜을 사용합니다.
...
보안: 외부 파서는 타임아웃, 환경 변수 제거, Linux 네트워크 격리로 실행됩니다. `parsers.external: true` 설정이 필요합니다. 자세한 내용은 [CONTRIBUTING.md](CONTRIBUTING.md)를 참조하세요.
...
스킬 파일은 범용 기본 템플릿을 제공합니다 — 언제 검색하고, 무엇을 캡처하고, 어떻게 쿼리할지 — config.yaml의 엔티티 및 관계 유형을 사용합니다. 도메인별 에이전트 동작(연구 트리거, 회의 캡처 패턴 등)을 위해 [기여 팩](#기여-팩)을 적용하세요:

```bash
sage-wiki init --skill claude-code --pack academic-research
```

팩의 `skills/` 디렉토리가 기본 스킬과 함께 도메인별 트리거를 추가합니다. `skill refresh`를 실행하면 표시된 스킬 섹션만 재생성합니다 — 다른 내용은 보존됩니다.
...
- **팩:** 기여 팩 시스템 — 8개 번들 팩, Git 기반 레지스트리, 설치/적용/제거/업데이트 라이프사이클, 스냅샷 롤백이 포함된 트랜잭션 적용.
- **외부 파서:** stdin/stdout 서브프로세스 프로토콜을 통한 런타임 플러그형 파일 형식 파서. 타임아웃, 환경 제거, 네트워크 격리로 샌드박스 실행.
